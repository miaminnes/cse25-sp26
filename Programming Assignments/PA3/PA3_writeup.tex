...
graded from the written response prompts in the notebook.
...
\begin{itemize}
  ...
  \item Suggested debug baseline: \texttt{episodes=10000}, \texttt{alpha=0.5}, \texttt{gamma=0.9}, \texttt{epsilon=0.1}.
\end{itemize}
...
\begin{itemize}
  ...
  \item In Question 2, update the agent's last \texttt{(state, action)} after the opponent moves, not immediately after the agent moves.
  \item Run each nearby checkpoint cell after finishing a TODO.
  \item Submit after each question, because earlier functions are used in later ones.
\end{itemize}
